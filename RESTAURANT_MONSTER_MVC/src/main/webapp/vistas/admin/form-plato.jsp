...
<div class="contenedor contenedor-medio" style="margin-top:25px;">
    ...
    <form action="${pageContext.request.contextPath}/admin" method="post" enctype="multipart/form-data">
        ...
        <div class="form-row">
            ...
            <div class="form-grupo">
                <label>Foto del Plato <%= esEdicion ? "" : "*" %></label>
                <input type="file" name="foto" class="form-control" accept="image/png, image/jpeg, image/webp" <%= esEdicion ? "" : "required" %>>
                
                <% if (esEdicion && plato.getFoto() != null) { %>
                    <input type="hidden" name="fotoActual" value="<%= plato.getFoto() %>">
                    <small style="display:block; margin-top:5px; color:#666;">Deja en blanco si no deseas cambiar la imagen actual.</small>
                <% } %>
            </div>
        </div>
